...
ggplot(data = mapdata) + 
  ...
  theme(text = element_text(family = "oswald"),
        title = element_text(family = "oswald", face = "bold")) +
...
:::
:::{.column width=50%}
![](../images/vandenberg.jpg){width=500px fig-align="center"}
:::
::::
...
![A photo of President Truman signing the European Recovery Act, also known as the Marshall Plan, into law. To Truman's right are Senator Vandenberg and Undersecretary of State Robert Lovett.](../images/marshall-plan-sign.jpg){width=900px height=600px fig-align="center"}



## {.blank-slide}

```{r Bipartisanship in Cong Voting}
#| fig-align: "center"
#| out-width: "75%"
...
ggplot(moderates.party |>  filter(name == "moderate.prop"), aes(x = congress, y = value, group = name, color = name)) +
  ...
  labs(x = "Congress",
       y = "Proportion",
       title = "Moderates as a percentage of members of Congress, 55th-116th Congress",
...
![A screenshot from Hiscox, Michael. 1999. "The Magic Bullet? The RTAA, Institutional Reform, and Trade Liberalization. International Organization. 53:4 669-698.](../images/hiscox-1999-figure.png){width=600px height=800px fig-align="center" style="margin-top:-75px;" fig-alt="Figure from Hiscox 1999 showing changes in party positions on trade over time."}

:::{.notes}
Notes

- From Hiscox (1999) this goes to show how important WWII was in changing Republican positions on trade
- Also note how Democrat positions drop in the 1970s and 1980s, but eventually rebound.
...
#  filter(congress == 55 | congress == 86 | congress == 115 | congress == 117) |> 
#  filter(chamber == "House") |> 
#  mutate(party = factor(party_code, levels = c(100, 200), labels = c("Democrat", "Republican")),
#         congress = factor(congress, labels = c("55th Congress (1897-1899)", "86th Congress (1947-1949)", "115th Congress #(2017-2019)", "117th Congress (2023-2025)"))) |> 
#  filter(!is.na(party)) |> 
#  dplyr::group_by(party, congress) |> 
#  dplyr::mutate(party_median = median(nominate_dim1, na.rm = TRUE))
#
#
#ggplot(nom.1, aes(x = nominate_dim1, fill = factor(party))) +
#  geom_density(alpha = 0.8) +
#  geom_vline(aes(xintercept = party_median),  color = "black", linetype = "dotted", size = 1) +
...
#       y = "Density",
#       fill = "Party ID",
#       title = "First Dimension House NOMINATE Scores across time",
...
ggplot(nom.1, aes(x = nominate_dim1, y = reorder(factor(congress), -congress), fill = factor(party))) +
  geom_density_ridges(alpha = 0.8, scale = 1.85, rel_min_height = 0.01, linewidth = 0.01) +
  geom_vline(xintercept = 0, size = 1.5, linetype = "dashed") +
  theme_flynn(base_family = "oswald", base_size = 11) + 
  ...
  labs(x = "First Dimension NOMINATE Score",
       y = "",
       fill = "Party ID",
       title = "First Dimension House NOMINATE Scores, 55th Congress-Present",
       caption = "Data obtained from Voteview.com")
