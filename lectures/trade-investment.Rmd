...
us.trade.gdp <- trade.national %>% 
  ...
  as.data.frame() %>% 
  rename("Exports" = "exports.gdp", "Imports" = "imports.gdp", "Exports (Constant)" = "exports.con", "Imports (Constant)" = "imports.con", "Deficit" = "deficit") %>% 
  pivot_longer(cols = c("Exports", "Imports", "Exports (Constant)", "Imports (Constant)", "Deficit", "realgdp"))



# US exports and imports over time
ggplot(data = subset(us.trade.gdp, name == "Exports (Constant)" | name == "Imports (Constant)"), aes(x = year, y = value/1000, group = name, colour = name)) +
  geom_line(size = 2) +
  theme_flynn() +
  labs(x = "Year",
  ...
  viridis::scale_color_viridis(option = "magma", discrete = TRUE, guide_colourbar(title = "Group"))
