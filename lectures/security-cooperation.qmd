...
troop.data.total <- troopdata::get_troopdata() |> 
  dplyr::group_by(year) |> 
  dplyr::summarise(total_troops = sum(troops_ad, na.rm = TRUE))
  
troop.data.overseas <- troopdata::get_troopdata() |> 
  dplyr::filter(ccode != 2) |> 
  dplyr::group_by(year) |> 
  dplyr::summarise(overseas_troops = sum(troops_ad, na.rm = TRUE))

troop.data.combined <- troop.data.total |> 
  left_join(troop.data.overseas, by = "year") |> 
  mutate(percent_overseas = overseas_troops/total_troops)
...
ggplot(data = troop.data.combined |> dplyr::filter(year>=1953)) +
  geom_area(aes(x = year, y = total_troops, fill = "Total"), position = "identity", alpha = .8, linewidth = .1, color = "black") +
  geom_area(aes(x = year, y = overseas_troops, fill = "Overseas"), position = "identity", alpha = .8, linewidth = .1, color = "black") +
  geom_line(aes(x = year, y = percent_overseas*10000000), position = "identity", alpha = .8, linewidth = 1) +
  ...
  labs(x = "Year",
       ...
       caption = "Data obtained from {troopdata} package for R.")
...
alluvialplot <- ggplot(alluvial.data, aes(y = level, weight = level, axis1 = branch, axis2 = region, fill = branch)) +
  ...
  geom_label_repel(stat = "stratum", aes(label = ifelse(x == 1, after_stat(as.character(stratum)), NA), fill = branch, fontface = "bold"), size = 5, hjust = "left", nudge_x = -0.25, direction = "x", point.padding = 0, box.padding = 0.1, label.padding = 0.15, max.iter = Inf, show.legend = FALSE, alpha = 0.6) +
  geom_label_repel(stat = "stratum", aes(label = ifelse(x == 2, after_stat(as.character(stratum)), NA), fontface = "bold"), fill = "white", size = 5, hjust = "right", nudge_x = 0.38, direction = "y", point.padding = 0, box.padding = 0.1, label.padding = 0.15,  max.iter = Inf, min.segment.length = 0.15)+
...
![](../images/trump-nato-summit.jpg){width=500 height=300 fig-align="center"}
...
:::{.column width=50%}
![](../images/lord-ismay.jpg){width=400 height=550 fig-align="center"}
...
![](../images/ford-park-picture.jpg){width=600 height=400 fig-align="center"}
